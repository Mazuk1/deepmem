...
class EmbeddingBase(ABC):
    ...
    async def aembed(self, text,
        ...
        """Default async wrapper — offloads sync .embed to a worker thread.

        Subclasses that load a real local model (BGE-M3) override this to add
        a GPU-serializing lock around the call. The default implementation is
        good enough for tiny in-memory test embedders.
        """
        return await asyncio.to_thread(self.embed, text, memory_action)

    async def aembed_batch(self, texts, memory_action: str = "add"):
